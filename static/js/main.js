...
function play_audio(audio_id, speaker_id) {
   ...
   function swap_visibility(elem1, elem2){
        if ($(elem1).is(":visible")){
            $(elem1).hide();
            $(elem2).show();
        }
   ...
   function update_sentence(input_validation, sentence, sentences){
        for(x=0; x<=sentences.length; x++){
            if(sentences[x] == sentence)
                sentences[x] = $('#'+input_validation).val();
        }

        $('#'+input_validation).val(sentences.join(';'));
   }

   function add_sentence(input_validation, sentence_id, sentences){
        sentences.push($(sentence_id).val())
